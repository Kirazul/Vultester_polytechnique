...
const features = [
  {
    title: 'Chaînage avant',
    description: 'Moteur d\'inférence qui part des faits connus pour déduire de nouvelles conclusions.'
  },
  {
    ...
    <div className="min-h-screen pt-28 pb-16 px-6 bg-zinc-900 relative">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
          ...
          <h1 className="text-4xl font-semibold text-white mb-3">À propos</h1>
        ...
        <motion.div
          ...
          className="mb-20"
        >
          <h2 className="text-2xl font-medium text-white mb-8">Architecture</h2>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-8">
            <div className="flex items-center justify-between">
              {[
                { num: '1', label: 'Entrée', desc: 'Configuration' },
                { num: '2', label: 'Moteur', desc: 'Chaînage avant' },
                { num: '3', label: 'Base', desc: '50 règles' },
                { num: '4', label: 'Sortie', desc: 'Diagnostic' },
              ].map((item, i) => (
                <div key={item.num} className="flex items-center">
                  <div className="text-center">
                    <div className="w-16 h-16 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center mb-3">
                      <span className="text-blue-400 font-mono text-lg">{item.num}</span>
                    </div>
                    <div className="text-sm font-medium text-white">{item.label}</div>
                    <div className="text-xs text-zinc-500">{item.desc}</div>
                  </div>
                  {i < 3 && (
                    <div className="w-16 h-px bg-zinc-700 mx-4" />
                  )}
                </div>
              ))}
            </div>
